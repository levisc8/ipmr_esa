...
We will now create a `data.frame` to store the results for all of the vital rates, loop over them, and plot the results. We'll exclude $G(z', z)$ since it is more of a sub-kernel than a vital rate.
...
It seems that this iceplant population is very sensitive to survival, growth, and initial size. Looking at the plots above the kernel sensitivity surface, this makes sense given the kernel-level results. However, these transitions are also biologically unrealistic - ice plant cannot grow *that* fast!
...
The final quantity we'll compute is expected lifetime reproductive output given an initial size $z_0$. This computed as:
...
Next, we will get into how to create general IPMs using a data set on the plant species *Lupinus tidestromii*. These are also real data, collected from multiple sites in California!
